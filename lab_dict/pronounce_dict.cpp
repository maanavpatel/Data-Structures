...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    map<string, vector<string>>::const_iterator lk1 = dict.find(w1);
    map<string, vector<string>>::const_iterator lk2 = dict.find(w2);

    if (lk1 == dict.end() || lk2 == dict.end()) {
        return false;
    } else if (lk1->second != lk2->second) {
        return false;
    } else {
        return true;
    }
