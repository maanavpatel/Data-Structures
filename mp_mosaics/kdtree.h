...
class KDTree
{
  ...
  private:
     ...
     */
    double distance(const Point<Dim>& target,const Point<Dim>& p) const;
   
   typename KDTree<Dim>::KDTreeNode* buildTree(int start, int end,int currentDim);
    void quickSelect(int start, int end, int pivIndex, int currentDim);
    ...
    void destroy();
    void destroyNodes(typename KDTree<Dim>::KDTreeNode* subRoot);
};
